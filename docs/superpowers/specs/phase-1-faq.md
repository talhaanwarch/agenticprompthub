...
Drizzle ORM with the `postgres` npm driver. Drizzle is TypeScript-native (schema defined in `.ts` files, not a separate DSL), generates clean types for JSONB columns, and provides easy raw-SQL escape hatches via the `sql` tagged template — needed for RLS later. Prisma was considered but rejected: heavier runtime, separate schema language, harder JSONB ergonomics.

**Q: Where does the DB schema live?**
All Drizzle table definitions in `src/shared/db/schema.ts` — single source of truth. Migrations generated by drizzle-kit into `drizzle/` folder. Never edit generated migration SQL by hand.
